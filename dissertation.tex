...
\begin{ftsdef}
Suppose $F(t)$ is caused by $F(t-1),F(t-2),\ldots,F(t-n)$ simultaneously, then the fuzzy logical relationship for $F(t)$ can be represented by $F(t-n),\ldots, F(t-2),F(t-1) \rightarrow F(t)$. This is called an $n$th-order fuzzy time series.
\end{ftsdef}

An $n$th-order or higher-order fuzzy time series, introduced by \cite{chen2002forecasting}, is one where where there is a simultaneous relationship between multiple points on the time series, not just adjacent observations. [TODO: come back to this]
...
\cite{lee2006pattern} studies candlestick patterns over a fuzzy time series...

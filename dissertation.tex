...
This definition states that for every fuzzy logical relationship between $F(t)$ and $F(t-1)$, as in Definition 2, the relationships over the time series may be grouped in relationships where $F(t-1)$ are the same. Following from the previous example, if the values for $F(t)$ for a sequence of points on a time series are $low$, $high$, $med$, $low$, $low$ and $very\_high$, then the fuzzy logic relationships are $low \rightarrow high$, $high \rightarrow med$, $med \rightarrow low$, $low \rightarrow low$ and $low \rightarrow very\_high$. These can then be grouped into fuzzy logical relationship groups:

show fuzzy logic relations in seperate table

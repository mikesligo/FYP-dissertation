...
\cite{chen1996forecasting} later proposed a fuzzy time series model using simplified arithmetic operations instead of a complicated max-min composition. The model is defined as follows:
...
In this definition, $Y(t)$ represents a time series, with the function returning a value at different points along time $t$. The range of values in this time series is called the universe of discourse, over which a set of linguistic terms are defined. $f_i(t)$ are fuzzy sets of these linguistic terms, consisting of the all the linguistic terms and the membership of each term in that fuzzy set. $F(t)$ is the collection of all of these fuzzy sets. This collection of fuzzy sets at any point $t$ on a time series is called a fuzzy time series.
